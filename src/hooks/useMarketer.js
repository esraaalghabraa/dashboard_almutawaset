...
import { fetchMarketers, updateUserStatus } from "../stores/storeUser";
...
const fetchData = useCallback(async (params = {}) => {
    setLoading(true);
    try {
    const result = await fetchMarketers({ ...params, ...filters });
    setData(result.data);
...
const handleStatusChange = async (id, active) => {
    ...
    notification.success({
        message: 'Success',
        description: 'Marketer status updated successfully.',
    });
} catch (error) {
    notification.error({
        message: 'Error',
